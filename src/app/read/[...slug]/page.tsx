...
export default function ReadPage({ params }: ReadPageProps) {
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); // Mobile sheet
  ...
  useEffect(() => {
    ...
    if (savedMode) {
      setIsMushafMode(JSON.parse(savedMode));
    }
  }, []);
      ...
      <div className={cn(
        ...
        <div className="flex-1 overflow-auto">
          <div className="container mx-auto px-0 sm:px-0 lg:px-0">
            <QuranReader slug={slug} setCurrentPage={setCurrentPage} isMushafMode={isMushafMode} currentPage={currentPage} />
          </div>
        </div>
      </div>
    </div>
