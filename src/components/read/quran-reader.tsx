...
export function QuranReader({ slug, setCurrentPage, isMushafMode = false, currentPage }: QuranReaderProps) {
  const [isClient, setIsClient] = useState(false);
  const router = useRouter();

  /* Existing imports */

  /* Helper to get verses for a page */
  function getVersesForPage(surahId: number, page: number) {
    const pageInfo = quranPages.find(p => p.surah === surahId && p.page === page);
    if (!pageInfo) return null;
    return { from: pageInfo.from, to: pageInfo.to };
  }
  ...
  const visibleVerses = selectedSurah ? (isMushafMode ? (() => {
    ...
    if (pageRange) {
      return selectedSurah.verses.filter(v => {
        const vNum = parseInt(v.verse_key.split(':')[1]);
        return vNum >= pageRange.from && vNum <= pageRange.to;
      });
    }
  ...
  const handleScroll = useCallback(() => {
    if (!selectedSurah || isMushafMode) return;

    /* Removed auto-complete on scroll logic */
    const isAtBottom = window.innerHeight + window.scrollY >= document.body.offsetHeight - 5;
    /* 
       Logic removed per user request: 
       Previously checked isAtBottom to mark complete and jump to end. 
       Now manual only.
    */
  ...
  const handlePageChange = (newPage: number) => {
    ...
    /* Update progress ONLY on explicit page turn in Mushaf Mode */
    if (selectedSurah && isMushafMode) {
      // If already marked complete, don't revert it
      if (progress[selectedSurah.id] === 100) return;

      // Find range for new page
      const pageInfo = quranPages.find(p => p.surah === selectedSurah.id && p.page === newPage);

      if (pageInfo) {
        const lastVerseOnPage = pageInfo.to;
        const totalVerses = selectedSurah.total_verses;
        // Cap at 99% so "Complete" is manual
        const newProgress = Math.min(Math.round((lastVerseOnPage / totalVerses) * 100), 99);

        if (progress[selectedSurah.id] !== newProgress) {
          updateProgress(selectedSurah.id, newProgress);
        }
      }
    }
  };
          ...
          <div ref={readerContainerRef} className="p-6 md:p-8 lg:p-12 space-y-8 font-body text-lg">
            {isLoading ? (
              ...
              <>
                  ...
                  {currentVerseKey && (
                    <>
                      <Button variant="ghost" onClick={() => {
                        const v = selectedSurah?.verses.find(v => v.verse_key === currentVerseKey);
                        if (v) handleBookmarkToggle(v);
                      }}>
                        <Bookmark className={cn('mr-2 h-4 w-4', isBookmarked(currentVerseKey) ? 'fill-accent text-accent' : '')} />
                        {isBookmarked(currentVerseKey) ? 'Bookmarked' : 'Bookmark'}
                      </Button>
                      <Button variant="ghost" onClick={() => {
                        const v = selectedSurah?.verses.find(v => v.verse_key === currentVerseKey);
                        if (v) handleCopy(v);
                ...
                {isMushafMode ? (
                  <div className="font-arabic text-3xl md:text-4xl leading-[2.5] md:leading-[2.8] text-justify" dir="rtl">
                    {visibleVerses.map((verse: Verse) => (
                      <span
                        key={verse.verse_key}
                        ref={(el) => { verseRefs.current[verse.verse_key] = el; }}
                        className={cn(
                          "inline hover:bg-muted/50 transition-colors cursor-pointer px-1 rounded mx-0.5",
                          currentVerseKey === verse.verse_key ? "bg-primary/20" : "",
                          isBookmarked(verse.verse_key) ? "bg-amber-500/10 decoration-amber-500 decoration-2 underline-offset-4" : ""
                        )}
                        onClick={() => handleVerseClick(verse)}
                      >
                        {verse.arabic}
                        <span className="text-xl mx-1 text-primary font-sans border-2 border-primary rounded-full w-8 h-8 inline-flex items-center justify-center scale-75 align-middle select-none">
                            ...
                            <p className="text-3xl lg:text-4xl leading-relaxed text-right font-arabic" dir="rtl">
                              ...
                              {verse.verse_key && (
                                <>
                                  ...
                                  {!isMushafMode && <span dangerouslySetInnerHTML={{ __html: '&lrm;' }} />}
                                </>
                              )}
                            </p>
                            {!isMushafMode && (
